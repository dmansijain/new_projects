...
function GroupManageCtrl($scope, $compile, $http, $q, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $timeout, $routeParams, $window, $filter, $route){
   ...
   .then(function(response) {
    ...
    $scope.dtColumns = [
       ...
       DTColumnBuilder.newColumn('group_name').withTitle('Group Name'),
		 ...
		 DTColumnBuilder.newColumn(null).withTitle('Action').renderWith(function(data, type, full, meta) {
			var encrypted ;
			 encrypted = $scope.encryptStr(data.id);
			 var html = '<a title="Edit" class="EditFaIcons" href="#!/group-edit/'+encrypted+'"><i class="fa fa-pencil"></i></a><button title="Delete" class="btn btn-danger btn FaIcons" ng-click="deletegroup('+data.id+')"><i class="fa fa-trash"></i></button><a title="Send Mail" class="EditFaIcons" href="#!/group-mail/'+encrypted+'"><i class="fa fa-envelope"></i></a>';
	...
	function deletegroup(groupID) {
		if($window.confirm("Are you want to delete this Group?")) {
	   ...
	   .then(function(response) {
		   ...
		   if(response.data.success !='false'){
			    var Gritter = function () {
						$.gritter.add({
							title: "",
							text: "Group Event deleted successfully"
								});
							return false;
							}(); 
					$timeout($route.reload(), 2000);
   ...
   //console.log($scope.selectedItem);
    ...
    $scope.dtColumns = [
        ...
        DTColumnBuilder.newColumn(null).withTitle('Roster Name').renderWith(function(data, type, full, meta) {
              if(data.group_id) {
				  return data.full_name + ' - ' + data.event_typename + ' , ' + $filter('date')(data.start_date, "MM/dd/yyyy");
			  } else {
		...
		DTColumnBuilder.newColumn(null).withTitle('Status').renderWith(function(data, type, full, meta) {
		 ...
		 DTColumnBuilder.newColumn(null).withTitle('Payment Status').renderWith(function(data, type, full, meta) {
			var rest_amt = data.cost-data.paid_amount;
            if(rest_amt == '0'){
				return "Completed";
			} else {
				return 'Pending';
   ...
   .then(function(response) {
	...
	$scope.getEventtypelist = function () {
      if ($scope.eventinfo.event_category !== "") {
		  ...
		  var formData = {'cat_id': $scope.eventinfo.event_category};
       $http.post(BASE_URL+'api/event/getalltypeInsteadgroup', formData)
   .then(function(response) {
      $scope.alltype = response.data.data; 
    });	
      }
